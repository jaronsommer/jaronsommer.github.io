...
- **`.claude/readme-reminder.ps1`** (committed): der Hook. Macht **drei Checks** bei jeder Datei-Änderung: (a) reminded README-Update wenn eine Nicht-README/Nicht-`.claude/`-Datei geändert wird; (b) reminded `optimize-images.ps1` wenn irgendein Bild in `images/` >150 KB ist; (c) reminded bei `.html`-Änderungen, `sitemap.xml` (`<lastmod>` aufs aktuelle Datum, neue Seiten als zusätzliche `<url>`) und ggf. `robots.txt` aktuell zu halten. Reminders kumulieren. **Hinweis: Windows-only** (PowerShell). Auf macOS/Linux würde der Hook fehlschlagen — bei Cross-Platform-Bedarf umschreiben.
